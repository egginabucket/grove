...
def register_translated_lexicon(path: str, lang: Lang, native_lang=Lang.native()):
    with open(path) as f:
        LexemeTranslation.objects.bulk_create(LexemeTranslation(
            lexeme=LexemeTranslation.objects.get(
                lang=native_lang, term=native_term).lexeme,
            term=translated_term,
            lang=lang,
        ) for native_term, translated_term in yaml.load(f.read(), settings.YAML_LOADER))


def register_full_lexicon(path=LEXICON_PATH, native_lang=Lang.native(), clear=True):
    if clear:
    ...
    for fn in os.listdir(path):
        ...
        if os.path.isfile(full_path) and full_path.endswith('.yaml'):
            ...
            register_translated_lexicon(full_path,
                                        Lang.objects.get(code=lang_code),
                                        native_lang,
                                        )
